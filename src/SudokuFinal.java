...
public class SudokuFinal extends JFrame implements ActionListener{
   ...
   public static  Color OPEN_CELL_BGCOLOR =  new Color(222,184,135);  //light brown (open cell)
   public static final Color OPEN_CELL_TEXT_YES = Color.BLACK;  // RGB 
   public static final Color OPEN_CELL_TEXT_NO = Color.RED; 
   public static  Color CLOSED_CELL_BGCOLOR = new Color(210,105,30); // dark brown (closed cell)
   ...
   public static Color OPEN_CELL_PINKBG = Color.PINK;
   public static Color CLOSED_CELL_PINKBG = new Color(255, 20, 147);
   
   public static Color OPEN_CELL_YELLOWBG = new Color(240, 230, 140);
   public static Color CLOSED_CELL_YELLOWBG = Color.YELLOW;
   
   public static Color OPEN_CELL_GRAYBG = new Color(211, 211, 211);
   public static Color CLOSED_CELL_GRAYBG = Color.GRAY;
   
   public static Color OPEN_CELL_CYANBG = new Color(127, 255, 212);
   public static Color CLOSED_CELL_CYANBG = Color.CYAN;
   
   public static Color OPEN_CELL_ORIBG = new Color(222,184,135);
   public static Color CLOSED_CELL_ORIBG = new Color(210,105,30);
   
   public static Color OPEN_CELL_WHITEBG = Color.GRAY;
   public static Color CLOSED_CELL_WHITEBG = Color.WHITE;
   ...
   private BG1Final background = new BG1Final();  
   private BG2 background2 = new BG2();
   private BG3Final background3 = new BG3Final();
   private NoSolutionBGFinal nosolutionbg = new NoSolutionBGFinal(); 
   ...
   private JMenuItem original = new JMenuItem("Original");
...
//----------------------------------------------------------------------------------------------------------------------------------------------------------------------//
   ...
   public SudokuFinal() {
	   ...
	   Container cp = getContentPane();
	      ...
	      cp.add(new ScreenFinal(), BorderLayout.NORTH);  //create a new panel to store the timer.
	      //Title icon.
   ...
   //--------------------------------------------------------------------------------------------------------------------------------------------------------------//   
      ...
      theme.addActionListener(this);				//Theme
      pink.addActionListener(this);					//Pink
      yellow.addActionListener(this);				//Yellow
      grey.addActionListener(this);					//Grey
      cyan.addActionListener(this);					//Cyan
      original.addActionListener(this);
      white.addActionListener(this); 				//White
   ...
   public void actionPerformed(ActionEvent e) {
	...
	if(e.getSource() == easy) {
		...
		JOptionPane.showMessageDialog(null, "Welcome\n"+"to\n"+ "Easy Mode" ,"Sudoku (Easy)",JOptionPane.WARNING_MESSAGE,icon);
		...
		getPuzzle();
		getMasks(level);
		SudokuFinal.this.dispose();
		new SudokuFinal();
		background.disable();
		background2.disable();
		background3.disable();
		ImageIcon icon = new ImageIcon("D:\\Java\\new\\Sudoku\\int.gif ");
		JOptionPane.showMessageDialog(null, "Welcome\n"+ "to\n"+ "Intermediate Mode" ,"Sudoku (Intermediate)",JOptionPane.PLAIN_MESSAGE,icon);
		...
		ImageIcon icon = new ImageIcon("D:\\Java\\new\\Sudoku\\angry (3).gif ");
		
		JOptionPane.showMessageDialog(null,"Welcome\n"+ "to\n" + "Hard Mode" ,"Sudoku (Hard)",JOptionPane.PLAIN_MESSAGE,icon);
...
//Sound Control - Enable/Disable
		...
		OPEN_CELL_BGCOLOR = OPEN_CELL_PINKBG;
		CLOSED_CELL_BGCOLOR = CLOSED_CELL_PINKBG;
		for(int row = 0; row < GRID_SIZE; ++row) {
			for(int col = 0; col < GRID_SIZE; ++col) {
				if (masks[row][col]) {
		...
		OPEN_CELL_BGCOLOR = OPEN_CELL_YELLOWBG;
		CLOSED_CELL_BGCOLOR = CLOSED_CELL_YELLOWBG;
		for(int row = 0; row < GRID_SIZE; ++row) {
			for(int col = 0; col < GRID_SIZE; ++col) {
				if (masks[row][col]) {
		...
		OPEN_CELL_BGCOLOR = OPEN_CELL_GRAYBG;
		CLOSED_CELL_BGCOLOR = CLOSED_CELL_GRAYBG;
		for(int row = 0; row < GRID_SIZE; ++row) {
			for(int col = 0; col < GRID_SIZE; ++col) {
				if (masks[row][col]) {
		...
		OPEN_CELL_BGCOLOR = OPEN_CELL_CYANBG;
		CLOSED_CELL_BGCOLOR = CLOSED_CELL_CYANBG;
		for(int row = 0; row < GRID_SIZE; ++row) {
			for(int col = 0; col < GRID_SIZE; ++col) {
				if (masks[row][col]) {
		...
		OPEN_CELL_BGCOLOR = OPEN_CELL_ORIBG;
		CLOSED_CELL_BGCOLOR = CLOSED_CELL_ORIBG;
		for(int row = 0; row < GRID_SIZE; ++row) {
			for(int col = 0; col < GRID_SIZE; ++col) {
				if (masks[row][col]) {
		...
		OPEN_CELL_BGCOLOR = OPEN_CELL_WHITEBG;
		CLOSED_CELL_BGCOLOR = CLOSED_CELL_WHITEBG;
		for(int row = 0; row < GRID_SIZE; ++row) {
			for(int col = 0; col < GRID_SIZE; ++col) {
				if (masks[row][col]) {
   ...
   public static void main(String[] args) {
      ...
      SwingUtilities.invokeLater(new Runnable(){
    	  ...
    	  public void run() {
    		  ...
    		  ImageIcon icon = new ImageIcon("D:\\Java\\new\\Sudoku\\247sudoku.jpg");
    		  JOptionPane.showMessageDialog(null,"","Sudoku",JOptionPane.PLAIN_MESSAGE,icon);
    		
    		  new SudokuFinal();
    		 
    		 final JOptionPane pane = new JOptionPane("Welcome to Sudoku\r\n" + "It is now on Easy mode, you may select different difficulty under File, New Game.");
    		    final JDialog d = pane.createDialog((JFrame)null, "Sudoku");
